#!/usr/bin/env python3
"""
auto_sip_pipeline_v23_noscipy.py

v23:
- Keep v19/v20 core behavior:
  * RAW linear exponents plot (rho + imag)
  * Optional logistic fit for logRho vs logS (no SciPy)
  * Logistic-only plot
  * Logistic diagnostics plot (with safe fallback if logistic not accepted)
  * INDEX "goal" plot (2-panels) using through-origin fits on:
      logRI  = log10(rho/rho_ref)      = logRho - logRho_ref
      logICI = log10(sigma_ref/sigma) = logSig_ref - logImag   (FIXED)

- Add TARGET-style single-panel plots (match your goal figures):
  * TARGET_RI_exponent_<tag>Hz.png
  * TARGET_ICI_exponent_<tag>Hz.png

- Add per-run Results cleanup:
  * --clean-results  -> delete files inside each run's Results folder before writing outputs.
"""
...
# Logistic search bounds (k)
K_MIN = 0.05
K_MAX = 50.0

# Logistic acceptance rules
...
def nice_site_label(run_root: Path) -> str:
    # VN_S-12 -> "VN S 12" like your target figure titles
    s = run_root.name.replace("_", " ")
    s = s.replace("-", " ")
    s = " ".join(s.split())
    return s
...
def read_sip_output(sip_xlsx: Path, target_freq: float, max_df: float):
    ...
    for sh in xls.sheet_names:
        ...
        # fallback numeric assumptions (common SIP export pattern)
...
# ---------------- Results cleanup (NEW) ----------------
def delete_files_inside_results(results_dir: Path, verbose: bool=False):
    """
    Deletes files (and subfolders) inside results_dir, but keeps the Results directory itself.
    Safe-guard: only acts if the folder name is exactly 'Results' (case-insensitive).
    """
    if not results_dir.exists() or not results_dir.is_dir():
        return
    if results_dir.name.lower() != RESULTS_DIR.lower():
    ...
    for p in results_dir.iterdir():
        try:
            if p.is_file() or p.is_symlink():
                p.unlink()
                if verbose:
                    print(f"  deleted file: {p}", flush=True)
            elif p.is_dir():
                # recursive delete
                ...
                p.rmdir()
                if verbose:
                    print(f"  deleted dir:  {p}", flush=True)
        except Exception as e:
            raise RuntimeError(f"Failed cleaning {p}: {e}") from e
...
def write_plot_logistic_diag(results_dir: Path, csv_name: str, out_png: str, target_freq: float, lin_rho, log_fit_acc):
    """
    Always produces a valid plot.
    If logistic not accepted, panel 3 shows 'No logistic accepted' with a tiny dummy dataset.
    """
    label = f"{target_freq:g}"
...
# ----------- TARGET-style plots (NEW) -----------
def write_plot_target_ri(results_dir: Path, csv_name: str, out_png: str,
                         site_label: str, target_freq: float,
                         exp_val: float, r2_val: float,
                         a_ri: float):
    # Fit line is through-origin: y = a_ri * x, where exp = -a_ri
    label = f"{target_freq:g}"
    gp = f"""reset
set datafile separator ','
set datafile missing ''
set term pngcairo size 1200,900 enhanced font 'Arial,30'
set output '{out_png}'

set title '{site_label}: Saturation Exponent (Resistivity Index) at {label} Hz'
set grid
set key top right
set xlabel 'log10(Saturation)'
set ylabel 'log10(Resistivity Index)'
...
plot '{csv_name}' using 6:11 with points pt 7 ps 1.7 title 'data', \\
     f1(x) with lines dt 2 lw 3 title sprintf('exp=%.2f, R^2=%.3f', {exp_val:.12g}, {r2_val:.12g})
"""
    (results_dir / "plot_target_TARGET_RI_exponent.gp").write_text(gp)

def write_plot_target_ici(results_dir: Path, csv_name: str, out_png: str,
                          site_label: str, target_freq: float,
                          exp_val: float, r2_val: float,
                          a_ici: float):
    label = f"{target_freq:g}"
...
set title '{site_label}: Saturation Exponent (Imag Conductivity Index) at {label} Hz'
set grid
set key top right
set xlabel 'log10(Saturation)'
set ylabel 'log10(Imag Conductivity Index)'
...
plot '{csv_name}' using 6:12 with points pt 7 ps 1.7 title 'data', \\
     f1(x) with lines dt 2 lw 3 title sprintf('exp=%.2f, R^2=%.3f', {exp_val:.12g}, {r2_val:.12g})
"""
    (results_dir / "plot_target_TARGET_ICI_exponent.gp").write_text(gp)


# ---------------- discovery / gnuplot runner ----------------
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    if clean_results:
        delete_files_inside_results(results_dir, verbose=verbose_clean)
    ...
    # Logistic (optional) on RAW logRho vs logS
    log_fit = None
    log_status = "skipped"
    ...
    write_plot_logistic_diag(results_dir, csv_path.name, f"exponents_logistic_diagnostics_{tag}Hz.png", target_freq, lin_rho, log_fit_acc)

    # TARGET-style single panels
    site_label = nice_site_label(run_root)
    ...
    if not dry_run:
        ...
        _run_gnuplot("plot_target_TARGET_RI_exponent.gp", cwd=results_dir, timeout_s=gnuplot_timeout, fatal=True)
        _run_gnuplot("plot_target_TARGET_ICI_exponent.gp", cwd=results_dir, timeout_s=gnuplot_timeout, fatal=True)
...
def main():
    ...
    ap.add_argument("--verbose-clean", action="store_true",
                    help="Print deleted files when using --clean-results.")
