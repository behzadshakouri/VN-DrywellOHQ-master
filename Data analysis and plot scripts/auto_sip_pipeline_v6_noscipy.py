#!/usr/bin/env python3
"""
auto_sip_pipeline_v6_noscipy.py

Fix vs v5:
- gnuplot linear lines now use fitted intercepts (b1, b2) instead of forcing 0
  -> dashed line overlays data correctly.
...
- Writes ALL outputs into run_root/Results/ ONLY
  * joined_<freq>.csv
...
Usage:
  python3 auto_sip_pipeline_v6_noscipy.py "/path/to/top" --freq 0.01
Options:
...
def write_gnuplot(results_dir: Path, csv_name: str, out_png: str,
                 ...
                 log_fit):
    """
    lin_rho: dict {slope, intercept, r2, p, n}
    lin_im : dict {slope, intercept, r2, p}
    log_fit: dict or None
    """
    a_r = lin_rho["slope"]
    b_r = lin_rho["intercept"]   # ✅ use intercept
    r2_r = lin_rho["r2"]
    p_r = lin_rho["p"]

    a_i = lin_im["slope"]
    b_i = lin_im["intercept"]    # ✅ use intercept
    r2_i = lin_im["r2"]
    p_i = lin_im["p"]
...
def main():
    ...
    for p in base.rglob("*"):
        if p.is_dir() and (p/ANALYSIS_DIR).is_dir() and (p/RAW_DIR).is_dir():
            try:
                ...
                rows.append({"run": str(p), "status": "FAIL", "reason": str(e), "points": 0})
                print(f"[FAIL] {p}: {e}")
    ...
    print(f"Done. Visited {visited} run folder(s).")
