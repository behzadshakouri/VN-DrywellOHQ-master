#!/usr/bin/env python3
"""
auto_sip_pipeline_v26_noscipy.py

v26:
- FIX: remove f-string backslash SyntaxError by building gnuplot scripts using .format() only.
- TARGET plot in ONE figure (2 panels) in the same style as your example.
- Keep FIXED ICI definition by default:
    ICI = sigma_ref / sigma_imag
    logICI = log10(sigma_ref) - log10(sigma_imag)
  but add --ici-mode to optionally flip to:
    ICI = sigma_imag / sigma_ref  (matches raw direction if sigma increases with S)
- --clean-results deletes files inside each run’s Results/ folder before writing.
- Diagnostics plot avoids "Skipping data file with no valid points" by plotting only numeric columns.
"""
...
a1 = {a1}
f1(x) = a1*x
plot '{csv}' using 6:11 with points pt 7 ps 1.8 lc rgb '#0066ff' title 'data', \
     f1(x) with lines dt 2 lw 3 lc rgb '#0066ff' title sprintf('{label}Hz: n = %.2f, R^2 = %.3f', {n_idx}, {r2_ri})
...
a2 = {a2}
f2(x) = a2*x
plot '{csv}' using 6:12 with points pt 7 ps 1.8 lc rgb '#dd0000' title 'data', \
     f2(x) with lines dt 2 lw 3 lc rgb '#dd0000' title sprintf('{label}Hz: p = %.2f, R^2 = %.3f', {p_idx}, {r2_ici})
...
f1(x)=a1*x+b1
{log_defs}
plot '{csv}' using 6:7 with points pt 7 ps 1.7 title 'data', \
...
b2={b2:.12g}
f2(x)=a2*x+b2
plot '{csv}' using 6:8 with points pt 7 ps 1.7 title 'data', \
...
def write_plot_index(results_dir: Path, csv_name: str, out_png: str, target_freq: float,
                     n_idx: float, r2_ri: float, p_idx: float, r2_ici: float):
    # keep as a compact 2-panel index plot (same as target but different title)
    label = "{:g}".format(target_freq)
    gp = """reset
set datafile separator ','
set datafile missing ''
...
a1 = {a1}
f1(x) = a1*x
plot '{csv}' using 6:11 with points pt 7 ps 1.7 title 'data', \
...
a2 = {a2}
f2(x) = a2*x
plot '{csv}' using 6:12 with points pt 7 ps 1.7 title 'data', \
...
def write_plot_logistic_diag(results_dir: Path, csv_name: str, out_png: str, target_freq: float, has_log: bool):
    ...
    extra_curve = ""
    if has_log:
        extra_curve = ", \\\n     '{csv}' using 6:16 with lines dt 1 lw 4 title 'logistic (accepted)'".format(csv=csv_name)

    if has_log:
        panel3 = "plot '{csv}' using 6:14 with points pt 7 ps 1.4 title 'residuals'\n".format(csv=csv_name)
        extra3 = ""
    else:
        extra3 = (
            "unset key\n"
...
set xlabel 'log10(Saturation)'
set ylabel 'log10(Resistivity)'
plot '{csv}' using 6:7 with points pt 7 ps 1.6 title 'data', \
     '{csv}' using 6:15 with lines dt 2 lw 3 title 'linear'{extra_curve}
...
set xlabel 'log10(Saturation)'
set ylabel 'residual'
plot '{csv}' using 6:13 with points pt 7 ps 1.4 title 'residuals'
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    # Your convention: show n,p as positive exponents
    n_idx = -a_ri
    p_idx = -a_ici
    ...
    report.write_text(
        ...
        f"RAW linear logRho=a*logS+b: a={slope_r:.6g}, b={intercept_r:.6g}, n(raw)={-slope_r:.6g}, R2={r2_r:.6g}\n"
        f"RAW linear logImag=p*logS+q: p={slope_i:.6g}, q={intercept_i:.6g}, R2={r2_i:.6g}\n\n"
...
def main():
    ...
    ap.add_argument("--ici-mode", type=str, default="sigref_over_sig",
                    ...
                    help="Index definition for ICI: sigref_over_sig (default, ICI=sigma_ref/sigma) "
                         "or sig_over_sigref (ICI=sigma/sigma_ref; matches raw direction if sigma increases with S).")
    args = ap.parse_args()
