#!/usr/bin/env python3
"""
auto_sip_pipeline_v19_noscipy.py

v19:
- Keep linear fits + optional logistic + separate logistic plots (like v16/v17).
- FIX red-line issue: Imaginary Conductivity INDEX uses sigma_ref / sigma_imag:
    ICI = sigma_ref / sigma
    logICI = log10(sigma_ref) - log10(sigma)
- Add "goal" index-exponent plot (blue+red) using THROUGH-ORIGIN fits:
    logRI  = a1*logS  => n(index) = -a1
    logICI = a2*logS  => p(index) = -a2

Robust plotting:
- joined CSV has headers, gnuplot uses column("name").
"""
        ...
        if dof > 0 and ss_res >= 0:
            ...
            if np.all(np.isfinite(diag)) and np.all(diag >= 0):
                ...
                if np.all(se > 0) and np.all(np.isfinite(se)):
                    tstat = params / se
                    for name, tj in zip(["L", "k", "x0", "c"], tstat):
...
def write_gnuplot_index_exponents(results_dir: Path, csv_name: str, out_png: str,
                                 target_freq: float,
                                 a_ri, r2_ri, n_exp,
                                 a_ici, r2_ici, p_exp):
    label_freq = f"{target_freq:g}"
    series1 = [
        f"'{csv_name}' using (column('logS')):(column('logRhoIdx')) with points pt 7 ps 1.7 title 'data'",
        f"f1(x) with lines dt 2 lw 3 title sprintf('{label_freq}Hz: n=%.2f, R^2=%.3f', {n_exp:.12g}, {r2_ri:.12g})"
    ]
    series2 = [
        f"'{csv_name}' using (column('logS')):(column('logImagIdx')) with points pt 7 ps 1.7 title 'data'",
        f"f2(x) with lines dt 2 lw 3 title sprintf('{label_freq}Hz: p=%.2f, R^2=%.3f', {p_exp:.12g}, {r2_ici:.12g})"
    ]
...
def write_gnuplot_raw_exponents(results_dir: Path, csv_name: str, out_png: str,
    ...
    ser1 = [f"'{csv_name}' using (column('logS')):(column('logRho')) with points pt 7 ps 1.7 title 'data'"]
    ser1.append(
    ...
    if log_fit_accepted is not None:
        ...
        if np.isfinite(r2_log) and np.isfinite(p_model):
            ser1.append(
                "fL(x) with lines dt 1 lw 4 title sprintf('logistic: R^2=%.3f, p=%.3g', "
                f"{r2_log:.12g}, {p_model:.12g})"
            )
        else:
            ser1.append("fL(x) with lines dt 1 lw 4 title 'logistic fit'")

    ser2 = [f"'{csv_name}' using (column('logS')):(column('logImag')) with points pt 7 ps 1.7 title 'data'"]
    ser2.append(
...
def write_gnuplot_logistic_only(results_dir: Path, csv_name: str, out_png: str,
    ...
    label_freq = f"{target_freq:g}"
    ser = [f"'{csv_name}' using (column('logS')):(column('logRho')) with points pt 7 ps 1.7 title 'data'"]
    log_defs = ""
    if log_fit_accepted is not None:
        L = log_fit_accepted["L"]; k = log_fit_accepted["k"]; x0 = log_fit_accepted["x0"]; c = log_fit_accepted["c"]
        r2_log = log_fit_accepted.get("r2", np.nan)
...
def write_gnuplot_logistic_diagnostics(results_dir: Path, csv_name: str, out_png: str,
    ...
    panel1 = (
        "set xrange [*:*]\nset yrange [*:*]\n"
        f"plot '{csv_name}' using (column('logS')):(column('logRho')) with points pt 7 ps 1.6 title 'data', \\\n"
    ...
    panel2 = (
        "set xrange [*:*]\nset yrange [*:*]\n"
        f"plot '{csv_name}' using (column('logS')):((column('logRho')) - f1(column('logS'))) "
        f"with points pt 7 ps 1.4 title 'residuals'\n"
    )

    if has_log:
        panel3 = (
            "set xrange [*:*]\nset yrange [*:*]\n"
            f"plot '{csv_name}' using (column('logS')):((column('logRho')) - fL(column('logS'))) "
            f"with points pt 7 ps 1.4 title 'residuals'\n"
        )
        extra3 = ""
    else:
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    # Reference point: max S (closest to S=1)
    iref = int(np.nanargmax(sip_df["S"].to_numpy(float)))
    rho_ref  = float(sip_df.iloc[iref]["rho"])
    imag_ref = float(sip_df.iloc[iref]["sigma_imag"])
    logRho_ref  = safe_log10(rho_ref)
    logImag_ref = safe_log10(imag_ref)

    # INDEX (FIXED)
    sip_df["rho_idx"]    = sip_df["rho"] / rho_ref
    sip_df["imag_idx"]   = imag_ref / sip_df["sigma_imag"]   # <-- FIX

    sip_df["logRhoIdx"]  = sip_df["logRho"] - logRho_ref
    sip_df["logImagIdx"] = logImag_ref - sip_df["logImag"]   # <-- FIX
    ...
    txt = (
        ...
        f"rho_ref={rho_ref:.6g}, sigma_ref={imag_ref:.6g}\n\n"
    ...
    if log_fit is None:
        ...
        txt += (
            f"--- Logistic 4p fit on RAW logRho vs logS ---\n"
            f"status = {log_status}\n"
            f"L  = {log_fit['L']:.6g}\n"
            f"k  = {log_fit['k']:.6g}\n"
            f"x0 = {log_fit['x0']:.6g}\n"
    ...
    write_gnuplot_index_exponents(results_dir, csv_path.name, out_png_index, target_freq,
                                 a_ri, r2_ri, n_idx, a_ici, r2_ici, p_idx)

    out_png_raw = f"exponents_raw_{tag}Hz.png"
    write_gnuplot_raw_exponents(results_dir, csv_path.name, out_png_raw, target_freq,
