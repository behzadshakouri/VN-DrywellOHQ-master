#!/usr/bin/env python3
"""
auto_sip_pipeline_v15_noscipy.py

Fixes vs v14:
- FIX: plot_logistic_diagnostics.gp no longer uses `plot 0/0 notitle` when logistic
  is not accepted. That caused: "all points y value undefined!"
  Now it draws a dummy point from inline data (always valid) + a label.
- Diagnostics gnuplot remains NON-FATAL (warn and continue).
"""
...
def write_gnuplot_logistic_diagnostics(results_dir: Path, csv_name: str, out_png: str,
    ...
    panel2 = f"plot '{csv_name}' using 5:($6 - f1($5)) with points pt 7 ps 1.4 title 'residuals'\n"

    # KEY FIX:
    # If logistic not accepted, DO NOT plot an all-undefined dataset.
    # Instead: add a label + plot a dummy point from inline data (always valid).
    if has_log:
        panel3 = f"plot '{csv_name}' using 5:($6 - fL($5)) with points pt 7 ps 1.4 title 'residuals'\n"
        extra3 = ""
    else:
        extra3 = (
            "unset key\n"
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    if not dry_run:
        ...
        # Diagnostics is "extra" => NON-FATAL
