...
def _logistic_init_guess(x, y):
    """
    Fast heuristic guess for 4-parameter logistic:
      y ~ c + L/(1+exp(-k(x-x0)))

    Works well for monotonic-ish curves; good enough to seed local search.
    """
    ...
    if len(x) >= 3:
        ...
        # logistic slope at x0 is ~ L*k/4 => k ~ 4*slope/L
...
def logistic_fit_noscipy(x, y, fast=False, max_seconds=0.0):
    ...
    Returns (fit_dict, status_str)
      status_str in {"ok","timeout","fail","not_enough_points"}
    """
    x = np.asarray(x, float); y = np.asarray(y, float)
    m = np.isfinite(x) & np.isfinite(y)
    x = x[m]; y = y[m]
    ...
    # FAST path: no coarse grid; just init guess + local pattern search
    if fast:
                    ...
                    for dx0 in [0, -stepx0, stepx0]:
                        for dc in [0, -stepc, stepc]:
                            ...
                            if s2 + 1e-12 < best:
                                ...
                                improved = True
            # shrink steps regardless; this keeps it fast
        ...
        return {"L": Lb, "k": kb, "x0": x0b, "c": cb, "r2": r2, "p_model": p_model, "pvals": pvals}, "ok"

    # SLOW path (kept for completeness): coarse grid + refine
    # (You can still use it by NOT passing --logistic-fast)
    ...
    r2 = 1.0 - ss_res/ss_tot if ss_tot > 0 else np.nan
    p_model = f_test_pvalue(ss_res, ss_tot, df_model=3, n=n)
    return {"L": Lb, "k": kb, "x0": x0b, "c": cb, "r2": r2, "p_model": p_model, "pvals": {}}, "ok"
