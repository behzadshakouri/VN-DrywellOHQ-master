...
def write_gnuplot(results_dir: Path, csv_name: str, out_png: str,
    ...
    # Always plot points. Fit lines only if coefficients are finite.
    panel1_lines = []
    panel1_lines.append(f"plot '{csv_name}' using 5:6 with points pt 7 ps 1.7 title 'data'")
    if have_lin_r and np.isfinite(r2_r) and np.isfinite(p_r):
        panel1_lines.append(
            f"     , f1(x) with lines dt 2 lw 3 title sprintf('linear: n=%.2f, R^2=%.3f, p=%.3g', -a1, {r2_r:.12g}, {p_r:.12g})"
        )
    elif have_lin_r:
        panel1_lines.append("     , f1(x) with lines dt 2 lw 3 title 'linear fit'")
    if log_fit is not None:
        ...
        if is_finite(L,k,x0,c):
            ...
            if np.isfinite(r2_log) and np.isfinite(p_model):
                ...
                panel1_lines.append("     , fL(x) with lines dt 1 lw 4 title 'logistic fit'")

    panel1_plot = "\n".join(panel1_lines) + "\n"

    panel2_lines = []
    panel2_lines.append(f"plot '{csv_name}' using 5:7 with points pt 7 ps 1.7 title 'data'")
    if have_lin_i and np.isfinite(r2_i) and np.isfinite(p_i):
        panel2_lines.append(
            f"     , f2(x) with lines dt 2 lw 3 title sprintf('linear: p=%.2f, R^2=%.3f, p=%.3g', a2, {r2_i:.12g}, {p_i:.12g})"
        )
    elif have_lin_i:
        panel2_lines.append("     , f2(x) with lines dt 2 lw 3 title 'linear fit'")
    panel2_plot = "\n".join(panel2_lines) + "\n"

    # If coefficients are missing, set dummy but do not plot the function.
