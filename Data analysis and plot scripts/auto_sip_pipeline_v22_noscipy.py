#!/usr/bin/env python3
"""
auto_sip_pipeline_v22_noscipy.py

v22:
- Matches TARGET/GOAL figures:
  * Adds two single-panel "TARGET" plots:
      TARGET_RI_exponent_<tag>Hz.png
      TARGET_ICI_exponent_<tag>Hz.png
    with styling similar to your examples (blue RI, red ICI; dashed fit).
- Keeps RAW linear + optional logistic + separate logistic-only plot.
- Keeps INDEX "goal" plot (blue/red) using through-origin fits on:
    logRI  = log10(rho/rho_ref)        = logRho - logRho_ref
    logICI = log10(sigma_ref/sigma_imag)= logSig_ref - logImag   (FIXED)
- Robust gnuplot:
  * Numeric columns only in expressions ($6,$7,...) to avoid header issues.
  * Diagnostics residual panels never rely on header-only expressions.
- Adds Results cleanup:
  * clear_results_files(results_dir): deletes files inside Results (keeps folder).
  * CLI: --clean-results to apply per-run before writing outputs.

Folders:
  <run_root>/Analysis        : has "*output*.xlsx"
  <run_root>/SIP_Raw_Data    : has "*Log Book*.xlsx"
  <run_root>/Results         : outputs written here

Example:
python3 auto_sip_pipeline_v22_noscipy.py "/mnt/.../SIP" \
  --freq 0.01 --min-points 4 --logistic-min-points 6 --logistic-fast --logistic-max-seconds 8 --clean-results
"""
...
def to_title_name(run_root: Path) -> str:
    # Make something like "VN S-12" or "Van Nuys S12" if folder has underscores/dashes
    name = run_root.name.replace("_", " ").replace("-", " ").strip()
...
def read_logbook_anysheet(log_xlsx: Path, sample_filter: str | None):
    ...
    for sh in xls.sheet_names:
        ...
        if sample_col:
            score += 1
        if (not sat_col) and (mw_col and por_col and vol_col):
            score += 3
        if score == 0:
            continue
...
def read_sip_output(sip_xlsx: Path, target_freq: float, max_df: float):
    ...
    for sh in xls.sheet_names:
        ...
        # fallback: numeric positions if headers differ
...
# ---------------- Results cleanup ----------------
def clear_results_files(results_dir: Path, verbose: bool = True):
    """
    Delete files inside Results directory (keeps the directory itself).
    Does NOT delete subdirectories; only removes regular files.
    """
    ...
    for p in results_dir.iterdir():
        try:
            ...
            if verbose:
                print(f"[WARN] Could not delete {p}: {e}", flush=True)
    if verbose and n > 0:
        print(f"[INFO] Cleared {n} file(s) in {results_dir}", flush=True)


# ---------------- Plot writers (gnuplot; numeric cols) ----------------
def write_plot_index(results_dir: Path, csv_name: str, out_png: str, run_title: str, target_freq: float,
...
def write_plot_target_ri(results_dir: Path, csv_name: str, out_png: str, run_title: str, target_freq: float,
                         a_ri, r2_ri, n_idx):
    label = f"{target_freq:g}"
    # Blue points/line similar to your TARGET_RI_exponent_0p01Hz.png
    gp = f"""reset
set datafile separator ','
set datafile missing ''
set term pngcairo size 1400,1000 enhanced font 'Arial,28'
set output '{out_png}'
...
set xlabel 'log10(Saturation)'
set ylabel 'log10(Resistivity Index)'
set title '{run_title}: Saturation Exponent (Resistivity Index) at {label} Hz'

set style line 1 lc rgb '#1f77b4' pt 7 ps 2.0 lw 2
set style line 2 lc rgb '#1f77b4' dt 2 lw 3

a1 = {a_ri:.12g}
f1(x) = a1*x

plot '{csv_name}' using 6:11 with points ls 1 title 'data', \\
     f1(x) with lines  ls 2 title sprintf('exp=%.2f, R^2=%.3f', {n_idx:.12g}, {r2_ri:.12g})
"""
    (results_dir / f"plot_target_TARGET_RI_exponent_{str(label).replace('.','p')}Hz.gp").write_text(gp)

def write_plot_target_ici(results_dir: Path, csv_name: str, out_png: str, run_title: str, target_freq: float,
                          a_ici, r2_ici, p_idx):
    label = f"{target_freq:g}"
    # Red points/line similar to your TARGET_ICI_exponent_0p01Hz.png
    gp = f"""reset
set datafile separator ','
set datafile missing ''
set term pngcairo size 1400,1000 enhanced font 'Arial,28'
...
set xlabel 'log10(Saturation)'
set ylabel 'log10(Imag Conductivity Index)'
set title '{run_title}: Saturation Exponent (Imag Conductivity Index) at {label} Hz'

set style line 1 lc rgb '#d62728' pt 7 ps 2.0 lw 2
set style line 2 lc rgb '#d62728' dt 2 lw 3

a1 = {a_ici:.12g}
f1(x) = a1*x

plot '{csv_name}' using 6:12 with points ls 1 title 'data', \\
     f1(x) with lines  ls 2 title sprintf('exp=%.2f, R^2=%.3f', {p_idx:.12g}, {r2_ici:.12g})
"""
    (results_dir / f"plot_target_TARGET_ICI_exponent_{str(label).replace('.','p')}Hz.gp").write_text(gp)
...
def write_plot_logistic_only(results_dir: Path, csv_name: str, out_png: str, target_freq: float, log_fit_acc):
    ...
    # Always plot data; logistic curve only if accepted.
    log_defs = ""
    if log_fit_acc is not None:
        L = log_fit_acc["L"]; k = log_fit_acc["k"]; x0 = log_fit_acc["x0"]; c = log_fit_acc["c"]
        r2_log = log_fit_acc.get("r2", np.nan)
        ...
        if np.isfinite(r2_log) and np.isfinite(p_model):
            fit_part = f"fL(x) with lines dt 1 lw 4 title sprintf('logistic: R^2=%.3f, p=%.3g', {r2_log:.12g}, {p_model:.12g})"
        else:
            fit_part = "fL(x) with lines dt 1 lw 4 title 'logistic fit'"
    else:
        fit_part = "1/0 notitle"
...
def write_plot_logistic_diag(results_dir: Path, csv_name: str, out_png: str, target_freq: float, lin_rho, log_fit_acc):
    label = f"{target_freq:g}"
    ...
    panel1 = (
        f"plot '{csv_name}' using 6:7 with points pt 7 ps 1.6 title 'data', \\\n"
    ...
    # Numeric-only expressions in residuals:
    panel2 = f"plot '{csv_name}' using 6:($7 - f1($6)) with points pt 7 ps 1.4 title 'residuals'\n"

    if has_log:
        panel3 = f"plot '{csv_name}' using 6:($7 - fL($6)) with points pt 7 ps 1.4 title 'residuals'\n"
        extra3 = ""
    else:
        extra3 = (
            "unset key\n"
...
def process_run(run_root: Path, target_freq: float, sample_filter: str | None,
    ...
    if clean_results:
        clear_results_files(results_dir, verbose=True)
    ...
    # Logistic attempt (on RAW logRho vs logS)
    log_fit = None
    log_status = "skipped"
    ...
    report.write_text(
        ...
        f"Target freq requested: {target_freq:g} Hz\n"
    ...
    run_title = to_title_name(run_root)

    write_plot_index(
        results_dir, csv_path.name, f"index_exponents_{tag}Hz.png", run_title, target_freq,
        a_ri, r2_ri, n_idx, a_ici, r2_ici, p_idx
    )

    write_plot_raw(
        results_dir, csv_path.name, f"exponents_raw_{tag}Hz.png", target_freq,
        lin_rho, lin_im, log_fit_acc
    )

    write_plot_logistic_only(
        results_dir, csv_path.name, f"exponents_logistic_only_{tag}Hz.png", target_freq, log_fit_acc
    )

    write_plot_logistic_diag(
        results_dir, csv_path.name, f"exponents_logistic_diagnostics_{tag}Hz.png", target_freq, lin_rho, log_fit_acc
    )

    # TARGET single-panel plots (your “goals”)
    write_plot_target_ri(
        results_dir, csv_path.name, f"TARGET_RI_exponent_{tag}Hz.png", run_title, target_freq,
        a_ri, r2_ri, n_idx
    )
    write_plot_target_ici(
        results_dir, csv_path.name, f"TARGET_ICI_exponent_{tag}Hz.png", run_title, target_freq,
        a_ici, r2_ici, p_idx
    )
    ...
    if not dry_run:
        ...
        # TARGET plot scripts have dynamic names
        _run_gnuplot(f"plot_target_TARGET_RI_exponent_{tag}Hz.gp", cwd=results_dir, timeout_s=gnuplot_timeout, fatal=True)
        _run_gnuplot(f"plot_target_TARGET_ICI_exponent_{tag}Hz.gp", cwd=results_dir, timeout_s=gnuplot_timeout, fatal=True)
...
def main():
    ...
    ap.add_argument("--clean-results", action="store_true",
                    help="Delete files inside each run's Results folder before writing new outputs.")
    args = ap.parse_args()
    ...
    for i, run in enumerate(roots, 1):
        ...
        try:
            ...
            dt = time.time() - ti
            print(
                f"[{i}/{len(roots)}] OK points={res.get('points',0)} "
