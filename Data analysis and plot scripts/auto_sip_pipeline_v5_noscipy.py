#!/usr/bin/env python3
"""
auto_sip_pipeline_v5_noscipy.py

Batch SIP pipeline (NO SciPy):
- Recursively finds run roots containing Analysis/ and SIP_Raw_Data/
- Reads SIP processed output Excel (Analysis/*output*.xlsx)
- Reads Log Book Excel (SIP_Raw_Data/*Log Book*.xlsx)
  * Auto-detects correct sheet with Measurements + saturation (first sheet often geometry-only)
  * If saturation column absent, can compute saturation from Mass Water + Porosity + Column Volume
- Joins by measurement id (e.g., M1/M2/...) and extracts rho + sigma_imag at target frequency
- Writes ALL outputs into run_root/Results/
  * joined_<freq>.csv
  * fit_report_<freq>.txt
  * plot_exponents.gp
  * exponents_<freq>.png
- Fits:
  * Linear on log-log (always)
  * Logistic 4-parameter on logRho vs logS ONLY when points >= 5 (else skipped)
- Produces a global summary CSV in the TOP folder you pass:
  * Results_Summary_<freq>.csv

Usage:
  python3 auto_sip_pipeline_v5_noscipy.py "/path/to/top" --freq 0.01
Options:
  --sample "VN_S-1"   (filter logbook rows by Sample column if present)
  --dry-run           (do not run gnuplot; still writes csv/report/gp)
  --min-points 4      (minimum matched points required to process a run; default 4)
  --max-df 0.0        (max allowed abs(freq_found - freq_target). 0.0 disables check)
"""
...
# If saturation column not present, compute S from these:
...
# -----------------------------
# F distribution survival function (no SciPy)
# -----------------------------
...
def linear_fit(x, y):
    ...
    return float(slope), float(intercept), float(r2), float(pval)

def logistic4(x, L, k, x0, c):
    # Numerically stable logistic: clip exponent input
    z = k*(x - x0)
...
def logistic_fit_noscipy(x, y):
    """
    4-parameter logistic fit by grid search + coordinate descent refinement.
    Returns dict or None if not enough points (need >= 5).
    """
    x = np.asarray(x, float); y = np.asarray(y, float)
    m = np.isfinite(x) & np.isfinite(y)
    x = x[m]; y = y[m]
    ...
    def refine(L,k,x0,c, stepL,stepk,stepx0,stepc):
    ...
    for _ in range(6):
        Lb,kb,x0b,cb, ss_res = refine(Lb,kb,x0b,cb, stepL,stepk,stepx0,stepc)
    ...
    # approximate parameter p-values (finite diff Jacobian; normal approx)
    ...
    return {"L": Lb, "k": kb, "x0": x0b, "c": cb, "r2": r2, "p_model": p_model, "pvals": pvals}

# -----------------------------
# Reading: logbook (any sheet)
# -----------------------------
def read_logbook_anysheet(log_xlsx: Path, sample_filter: str|None):
    ...
    if not sat_map:
        raise RuntimeError(f"Selected logbook sheet '{sh}' but mapped 0 saturation rows. Columns: {list(df.columns)}")
...
def read_sip_output(sip_xlsx: Path, target_freq: float, max_df: float):
    ...
    for sh in xls.sheet_names:
        ...
        if (not rho_col) or (not imag_col):
            # fallback: freq, imag, phase, real, resistivity
            if len(df.columns) >= 5:
        ...
        if max_df > 0 and abs(f_found - target_freq) > max_df:
            # skip this sheet (target freq not present closely enough)
            continue
...
def write_gnuplot(results_dir: Path, csv_name: str, out_png: str,
                 ...
                 log_fit):
    """
    lin_rho: dict with slope/intercept/r2/p/n
    lin_im : dict with slope/intercept/r2/p
    log_fit: dict or None
    """

    # Linear: logRho = a*logS + b, n = -a
    a_r = lin_rho["slope"]
    r2_r = lin_rho["r2"]
    p_r = lin_rho["p"]
    n = -a_r
    ...
    # Build plot command depending on logistic availability
    if log_fit is None:
        panel1_plot = (
            f"plot '{csv_name}' using 5:6 with points pt 7 ps 1.7 title 'data', \\\n"
            f"     f1(x) with lines dt 2 lw 3 title sprintf('linear: n=%.2f, R^2=%.3f, p=%.3g', -a1, {r2_r:.12g}, {p_r:.12g})\n"
        )
    else:
        L = log_fit["L"]; k = log_fit["k"]; x0 = log_fit["x0"]; c = log_fit["c"]
        r2_log = log_fit["r2"]; p_model = log_fit["p_model"]
        panel1_plot = (
            f"L1 = {L:.12g}\n"
            f"k1 = {k:.12g}\n"
            f"x01= {x0:.12g}\n"
            ...
            f"plot '{csv_name}' using 5:6 with points pt 7 ps 1.7 title 'data', \\\n"
            f"     f1(x) with lines dt 2 lw 3 title sprintf('linear: n=%.2f, R^2=%.3f, p=%.3g', -a1, {r2_r:.12g}, {p_r:.12g}), \\\n"
            f"     fL(x) with lines dt 1 lw 4 title sprintf('logistic: L=%.2f k=%.2f x0=%.2f c=%.2f, R^2=%.3f, p=%.3g', L1,k1,x01,c1, {r2_log:.12g}, {p_model:.12g})\n"
        )
...
# Panel 1: Resistivity
# --------------------
set title 'Resistivity vs Saturation (log-log) at {label_freq} Hz'
set grid
set key top right
...
# Panel 2: Imag cond
# --------------------
set title 'Imaginary Conductivity vs Saturation (log-log) at {label_freq} Hz'
set grid
set key top right
set xlabel 'log10(Saturation)'
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    # join: SIP sheet name should match logbook Measurements
    ...
    if len(sip_df) < min_points:
        # Still write the joined CSV for debugging
    ...
    lin_rho = {"slope": slope_r, "intercept": intercept_r, "r2": r2_r, "p": p_r, "n": -slope_r}
    lin_im  = {"slope": slope_i, "intercept": intercept_i, "r2": r2_i, "p": p_i}

    # logistic only if >= 5
    log_fit = logistic_fit_noscipy(sip_df["logS"], sip_df["logRho"])
    has_logistic = (log_fit is not None)
    ...
    txt = (
        ...
        f"R2 = {r2_r:.6g}\n"
        f"p(model) = {p_r:.6g}\n\n"
        f"--- Linear fit (logImag vs logS): logImag = p*logS + q ---\n"
        f"p = {slope_i:.6g}\n"
        f"q = {intercept_i:.6g}\n"
        f"R2 = {r2_i:.6g}\n"
        f"p(model) = {p_i:.6g}\n\n"
    ...
    if has_logistic:
        txt += (
            ...
            f"p(L)  ~ {log_fit['pvals']['L']:.6g}\n"
            f"p(k)  ~ {log_fit['pvals']['k']:.6g}\n"
            f"p(x0) ~ {log_fit['pvals']['x0']:.6g}\n"
            f"p(c)  ~ {log_fit['pvals']['c']:.6g}\n"
        )
    else:
        txt += "Logistic fit skipped (need >= 5 matched points).\n"

    report.write_text(txt)

    # plot
    out_png = f"exponents_{tag}Hz.png"
    write_gnuplot(results_dir, csv_path.name, out_png, target_freq, lin_rho, lin_im, log_fit)
    ...
    return {
        ...
        "points": int(len(sip_df)),
        "logbook_sheet": used_sheet,
        "freq_target": float(target_freq),
        "n_linear": float(lin_rho["n"]),
        "r2_rho_linear": float(r2_r),
        "p_rho_linear": float(p_r),
        "p_linear_imag": float(slope_i),
        "r2_imag_linear": float(r2_i),
        "p_imag_linear": float(p_i),
        "logistic_used": bool(has_logistic),
        "L": float(log_fit["L"]) if has_logistic else np.nan,
        "k": float(log_fit["k"]) if has_logistic else np.nan,
        "x0": float(log_fit["x0"]) if has_logistic else np.nan,
        "c": float(log_fit["c"]) if has_logistic else np.nan,
        "r2_logistic": float(log_fit["r2"]) if has_logistic else np.nan,
        "p_logistic_model": float(log_fit["p_model"]) if has_logistic else np.nan,
        "p_L": float(log_fit["pvals"]["L"]) if has_logistic else np.nan,
        "p_k": float(log_fit["pvals"]["k"]) if has_logistic else np.nan,
        "p_x0": float(log_fit["pvals"]["x0"]) if has_logistic else np.nan,
        "p_c": float(log_fit["pvals"]["c"]) if has_logistic else np.nan,
    }
...
def main():
    ...
    ap.add_argument("--sample", type=str, default=None, help="Optional sample filter if Sample column exists")
    ap.add_argument("--dry-run", action="store_true", help="Do not run gnuplot (still writes Results files)")
    ap.add_argument("--min-points", type=int, default=4, help="Min matched points to process a run (default 4)")
    ap.add_argument("--max-df", type=float, default=0.0,
                    help="Max allowed abs(freq_found - freq_target). 0 disables (default 0)")
    args = ap.parse_args()
    ...
    for p in base.rglob("*"):
        if p.is_dir() and (p/ANALYSIS_DIR).is_dir() and (p/RAW_DIR).is_dir():
            try:
                res = process_run(p, args.freq, args.sample, args.dry_run, args.min_points, args.max_df)
                if res is None:
                    continue
                rows.append(res)
                processed += 1
                if res["status"] == "OK":
                    print(f"[OK]   {p}  (points={res['points']}, logistic={res.get('logistic_used', False)})")
                else:
                    print(f"[FAIL] {p}: {res['reason']} (points={res['points']}, sheet={res.get('logbook_sheet','?')})")
    ...
    # Write global summary at TOP folder (not inside individual runs)
    ...
    print(f"Done. Visited {processed} run folder(s).")
