#!/usr/bin/env python3
"""
auto_sip_pipeline_v14_noscipy.py

Fixes vs v13:
- FIX: plot_logistic_diagnostics.gp used literal "{csv_name}" due to bad string build.
  Now always injects the real csv filename.
- Diagnostics gnuplot failure is NON-FATAL: warn and continue.
"""
...
def write_gnuplot_logistic_diagnostics(results_dir: Path, csv_name: str, out_png: str,
    ...
    # FIXED: always use the real csv_name, no stray "{csv_name}"
    label_freq = f"{target_freq:g}"
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    if not dry_run:
        # Combined + logistic_only are "core" => fatal if they fail
