#!/usr/bin/env python3
"""
auto_sip_pipeline_v8_noscipy.py

v8 improvements over v7:
- Logistic fitting can be the slow part (CPU=100% for minutes).
  Add controls:
    * --logistic-min-points (default 6): attempt logistic only if enough points
    * --logistic-fast: use smaller grids (much faster)
    * --logistic-max-seconds (default 10): time-limit logistic per run; if exceeded -> skip logistic
- Keeps: fast run-root discovery, progress prints, gnuplot timeout, fixed intercept plotting.

Usage:
  python3 auto_sip_pipeline_v8_noscipy.py "/path/to/top" --freq 0.01
Common:
  python3 auto_sip_pipeline_v8_noscipy.py "/path/to/top" --freq 0.01 --logistic-fast --logistic-min-points 6 --logistic-max-seconds 8
  python3 auto_sip_pipeline_v8_noscipy.py "/path/to/top" --freq 0.01 --no-logistic
"""
...
def logistic4(x, L, k, x0, c):
    z = k*(x - x0)
    z = np.clip(z, -60.0, 60.0)  # prevents exp overflow
...
def logistic_fit_noscipy(x, y, fast=False, max_seconds=0.0):
    ...
    - fast=True: smaller grids
    - max_seconds>0: hard time limit; returns None if exceeded
    Returns dict or None if not enough points (need >=5) or timeout.
    """
    x = np.asarray(x, float); y = np.asarray(y, float)
    m = np.isfinite(x) & np.isfinite(y)
    x = x[m]; y = y[m]
    ...
    # approximate parameter p-values
...
def main():
    ...
    ap.add_argument("--no-logistic", action="store_true",
                    help="Disable logistic fit entirely (fastest)")
    ap.add_argument("--logistic-min-points", type=int, default=6,
                    help="Only attempt logistic if matched points >= this (default 6)")
    ap.add_argument("--logistic-fast", action="store_true",
                    help="Use faster/coarser logistic grid")
    ap.add_argument("--logistic-max-seconds", type=float, default=10.0,
                    help="Max seconds allowed for logistic per run (default 10). 0 disables.")

    ap.add_argument("--gnuplot-timeout", type=int, default=30,
                    help="Seconds; 0 disables timeout. Default 30.")
    args = ap.parse_args()
    ...
    for i, run in enumerate(roots, 1):
        ...
        try:
            ...
            rows.append(res)
            dt = time.time() - ti
            print(f"[{i}/{len(roots)}] {res['status']} points={res.get('points',0)} logistic={res.get('logistic_used',False)} ({dt:.1f}s)",
                  flush=True)
