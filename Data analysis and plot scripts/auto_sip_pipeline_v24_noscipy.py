#!/usr/bin/env python3
"""
auto_sip_pipeline_v24_noscipy.py

v24 (fixed):
- RAW linear + optional logistic + separate logistic-only plot.
- INDEX “goal” plots (through-origin fits) for:
    logRI  = log10(rho/rho_ref)       = logRho - logRho_ref
    logICI = log10(sigma_ref/sigma)   = logSig_ref - logSig     (FIXED ICI)
- TARGET plot in ONE figure (2 panels) in the same style as your example
  (RI top blue, ICI bottom red), saved as: target_exponents_<tag>Hz.png
- --clean-results deletes files inside each run’s Results/ folder before writing.
- Fix gnuplot “Skipping data file with no valid points” in diagnostics by writing
  residual columns into the CSV (no gnuplot expressions needed).
"""
...
# --- TOP: RI ---
set title 'Van Nuys S12: Saturation Exponent (Resistivity Index) at {label} Hz'
...
# --- BOTTOM: ICI ---
set title 'Van Nuys S12: Saturation Exponent (Imag Conductivity Index) at {label} Hz'
set xlabel 'log10(Saturation)'
...
def write_plot_logistic_diag(results_dir: Path, csv_name: str, out_png: str, target_freq: float, has_log: bool):
    ...
    # IMPORTANT: build the optional extra plot clause OUTSIDE any f-string expression
    extra_logistic_curve = ""
    if has_log:
...
plot '{csv_name}' using 6:7 with points pt 7 ps 1.6 title 'data', \\
     '{csv_name}' using 6:15 with lines dt 2 lw 3 title 'linear'{extra_logistic_curve}
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    # Precompute residual columns (so gnuplot never needs expressions)
    ...
    report.write_text(
        ...
        f"INDEX defs: RI=rho/rho_ref; ICI=sigma_ref/sigma\n"
