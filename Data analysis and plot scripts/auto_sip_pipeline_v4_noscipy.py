#!/usr/bin/env python3
"""
auto_sip_pipeline_v4_noscipy.py
...
- AUTO-detects the correct logbook sheet/columns (geometry-only first sheet is common)
- Joins by measurement id (e.g., M1/M2/...) and uses/computes saturation
- Writes ALL outputs into run_root/Results/ (does not touch other folders)
- Produces CSV + fit report + gnuplot script + PNG

NO SciPy. Uses numpy/pandas + gnuplot.

Usage:
  python3 auto_sip_pipeline_v4_noscipy.py /path/to/top_parent --freq 0.01
Optional:
  --sample "VN S-1"   (filters logbook rows by Sample column if present)
  --dry-run           (do not run gnuplot)
"""
...
# --- Logbook column candidates (auto-detected) ---
...
# --- SIP output column candidates (also fallback by position) ---
...
# -------- Stats: F-test + F survival function (no SciPy) --------
...
# -------- Logistic fit (grid + refine; no SciPy) --------
...
# -------- Logbook reading: scan all sheets and pick best one --------
def read_logbook_anysheet(log_xlsx: Path, sample_filter: str|None):
    ...
    if not sat_map:
        raise RuntimeError(f"Selected logbook sheet '{sh}' but could not map any saturation rows. Columns: {list(df.columns)}")
...
def write_gnuplot(results_dir: Path, csv_name: str, out_png: str,
                  n_lin, r2_lin, p_lin,
                  p_im_lin, r2_im, p_im,
                  fit_log, label_freq: str):
    L = fit_log["L"]; k = fit_log["k"]; x0 = fit_log["x0"]; c = fit_log["c"]
    r2_log = fit_log["r2"]; p_model = fit_log["p_model"]
...
plot '{csv_name}' using 5:6 with points pt 7 ps 1.7 title 'data', \\
     ...
     fL(x) with lines dt 1 lw 4 title sprintf('logistic: L=%.2f k=%.2f x0=%.2f c=%.2f, R^2=%.3f, p=%.3g', L1,k1,x01,c1, {r2_log:.12g}, {p_model:.12g})
...
plot '{csv_name}' using 5:7 with points pt 7 ps 1.7 title 'data', \\
     f2(x) with lines dt 2 lw 3 title sprintf('linear: p=%.2f, R^2=%.3f, p=%.3g', a2, {r2_im:.12g}, {p_im:.12g})
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None, dry_run: bool):
    ...
    # join: SIP sheet name (e.g., M1) must match logbook measurement ID
    ...
    if len(sip_df) < 4:
        raise RuntimeError(f"Not enough matched points after join (need >=4). Got {len(sip_df)}. Logbook sheet used: '{used_sheet}'.")
    ...
    write_gnuplot(results_dir, csv_path.name, out_png,
                  n, r2_r, p_r,
                  slope_i, r2_i, p_i,
                  fit, f"{target_freq:g}")
    ...
    print(f"[OK] {run_root} -> {results_dir/out_png} (logbook sheet: {used_sheet})")
    return True
