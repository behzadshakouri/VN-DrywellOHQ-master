...
def logistic_fit_noscipy(x, y):
    ...
    # moderate grid (fast enough for batch)
...
def discover_run_roots_fast(base: Path):
    # MUCH faster than rglob("*"): just find Analysis dirs and take their parent
    roots = set()
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
                ...
                do_logistic: bool, gnuplot_timeout: int):
    ...
    if log_fit is None:
        txt += "Logistic fit skipped (disabled or need >= 5 matched points).\n"
    else:
        txt += (
...
def main():
    ...
    for i, run in enumerate(roots, 1):
        ...
        ti = time.time()
        try:
            res = process_run(run, args.freq, args.sample, args.dry_run,
            ...
            dt = time.time() - ti
            if res["status"] == "OK":
                print(f"[{i}/{len(roots)}] OK   points={res['points']} logistic={res['logistic_used']}  ({dt:.1f}s)",
                      flush=True)
            else:
                print(f"[{i}/{len(roots)}] FAIL {res['reason']}  ({dt:.1f}s)", flush=True)
