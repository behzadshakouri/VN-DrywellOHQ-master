#!/usr/bin/env python3
"""
auto_sip_pipeline_v16_noscipy.py

Fix vs v15:
- FIX: plot_logistic_diagnostics.gp fallback panel (when logistic not accepted)
  no longer triggers "x range is invalid".
  We now:
    * reset xrange/yrange before each panel,
    * and for the fallback panel plot TWO inline points with distinct x values,
      plus set a safe xrange (logS is always <= 0).
"""
...
def write_gnuplot_logistic_diagnostics(results_dir: Path, csv_name: str, out_png: str,
    ...
    if has_log:
        ...
        # Robust fallback:
        # - force a safe xrange (logS is <= 0; give room)
        # - provide TWO points so xmin != xmax
        extra3 = (
            "unset key\n"
