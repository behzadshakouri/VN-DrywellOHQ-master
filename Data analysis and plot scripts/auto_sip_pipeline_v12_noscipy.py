#!/usr/bin/env python3
"""
auto_sip_pipeline_v12_noscipy.py

Fixes vs v11:
1) Prevent degenerate logistic fits (flat line) by enforcing k >= K_MIN (default 0.05).
2) Make logistic p-value robust: if SSE >= SST -> p_model = 1.0 (not NaN).
3) No covariance warnings: only compute parameter pvals if covariance diag is valid.
4) Write an additional logistic-only plot (gnuplot .gp + .png):
   - plot_logistic_only.gp
   - exponents_logistic_only_<freq>.png

Usage:
  python3 auto_sip_pipeline_v12_noscipy.py "/path/to/top" --freq 0.01
  python3 auto_sip_pipeline_v12_noscipy.py "/path/to/top" --freq 0.01 --logistic-min-points 6 --logistic-fast --logistic-max-seconds 8
"""
...
# ---- logistic bounds to prevent flat fits ----
K_MIN = 0.05     # was ~1e-6 before -> flat curve; keep >= 0.05
K_MAX = 50.0
...
def f_test_pvalue(ss_res, ss_tot, df_model, n):
    # If model is worse than mean (SSE >= SST), treat as p = 1 (not significant)
...
def linear_fit(x, y):
    ...
    return float(slope), float(intercept), float(r2), float(pval), float(ss_res)
...
def _logistic_init_guess(x, y):
    ...
    # k from approximate slope near x0
    if len(x) >= 3:
...
def logistic_fit_noscipy(x, y, fast=False, max_seconds=0.0):
    """
    Returns (fit_dict, status_str) with status_str in:
      {"ok","timeout","fail","not_enough_points"}
    """
    x = np.asarray(x, float); y = np.asarray(y, float)
    m = np.isfinite(x) & np.isfinite(y)
    x = x[m]; y = y[m]
    ...
    # FAST path: heuristic + local pattern search
    if fast:
        ...
        # parameter p-values (optional, robust)
        ...
        return {
            ...
            "r2": float(r2), "p_model": float(p_model) if np.isfinite(p_model) else p_model,
            "pvals": pvals, "pvals_ok": pvals_ok,
            "ss_res": float(ss_res), "ss_tot": float(ss_tot)
        }, "ok"

    # Non-fast path kept minimal (optional)
    return None, "fail"
...
def write_gnuplot_combined(results_dir: Path, csv_name: str, out_png: str,
    ...
    if is_finite(a_r, b_r):
        ser1.append(
            f"f1(x) with lines dt 2 lw 3 title sprintf('linear: n=%.2f, R^2=%.3f, p=%.3g', -a1, {r2_r:.12g}, {p_r:.12g})"
        )

    log_defs = ""
    if log_fit is not None and is_finite(log_fit.get("L",np.nan), log_fit.get("k",np.nan), log_fit.get("x0",np.nan), log_fit.get("c",np.nan)):
        L = log_fit["L"]; k = log_fit["k"]; x0 = log_fit["x0"]; c = log_fit["c"]
        r2_log = log_fit.get("r2", np.nan)
        p_model = log_fit.get("p_model", np.nan)
        ...
        if np.isfinite(r2_log) and np.isfinite(p_model):
            ser1.append(
                "fL(x) with lines dt 1 lw 4 title sprintf('logistic: L=%.2f k=%.2f x0=%.2f c=%.2f, R^2=%.3f, p=%.3g', "
                f"L1,k1,x01,c1, {r2_log:.12g}, {p_model:.12g})"
            )
        else:
            ser1.append("fL(x) with lines dt 1 lw 4 title 'logistic fit'")

    ser2 = [f"'{csv_name}' using 5:7 with points pt 7 ps 1.7 title 'data'"]
    if is_finite(a_i, b_i):
        ser2.append(
...
def write_gnuplot_logistic_only(results_dir: Path, csv_name: str, out_png: str,
                               target_freq: float, log_fit):
    """
    Logistic-only plot: Resistivity vs Saturation (log-log) with logistic curve.
    If logistic not available, plot only points.
    """
    label_freq = f"{target_freq:g}"
    ser = [f"'{csv_name}' using 5:6 with points pt 7 ps 1.7 title 'data'"]
    log_defs = ""
    ...
    if log_fit is not None and is_finite(log_fit.get("L",np.nan), log_fit.get("k",np.nan), log_fit.get("x0",np.nan), log_fit.get("c",np.nan)):
        ...
        if np.isfinite(r2_log) and np.isfinite(p_model):
            ...
            ser.append("fL(x) with lines dt 1 lw 4 title 'logistic fit'")
...
set title 'Resistivity vs Saturation (log-log) at {label_freq} Hz (logistic only)'
set grid
set key top right
set xlabel 'log10(Saturation)'
set ylabel 'log10(Resistivity)'
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    slope_r, intercept_r, r2_r, p_r, ss_res_lin_r = linear_fit(sip_df["logS"], sip_df["logRho"])
    slope_i, intercept_i, r2_i, p_i, ss_res_lin_i = linear_fit(sip_df["logS"], sip_df["logImag"])

    lin_rho = {"slope": slope_r, "intercept": intercept_r, "r2": r2_r, "p": p_r, "n": -slope_r, "ss_res": ss_res_lin_r}
    lin_im  = {"slope": slope_i, "intercept": intercept_i, "r2": r2_i, "p": p_i, "ss_res": ss_res_lin_i}
    ...
    if log_fit is None:
        ...
        if not bool(log_fit.get("pvals_ok", False)):
            txt += "Parameter p-values: unavailable (singular/ill-conditioned)\n"
        else:
            pv = log_fit.get("pvals", {})
            txt += (
                f"p(L)={pv.get('L',np.nan):.6g}, "
                f"p(k)={pv.get('k',np.nan):.6g}, "
                f"p(x0)={pv.get('x0',np.nan):.6g}, "
                f"p(c)={pv.get('c',np.nan):.6g}\n"
            )
    ...
    write_gnuplot_combined(results_dir, csv_path.name, out_png, target_freq, lin_rho, lin_im, log_fit)

    out_png_log = f"exponents_logistic_only_{tag}Hz.png"
    write_gnuplot_logistic_only(results_dir, csv_path.name, out_png_log, target_freq, log_fit)
    ...
    if not dry_run:
        ...
        subprocess.run(["gnuplot", "plot_logistic_only.gp"], cwd=str(results_dir), check=True,
                       timeout=(gnuplot_timeout if gnuplot_timeout > 0 else None))
