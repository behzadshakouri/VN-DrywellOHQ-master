#!/usr/bin/env python3
"""
auto_sip_pipeline_v13_noscipy.py

v13 improvements over v12:
- Logistic is ACCEPTED only if it improves over linear (R2 or AIC).
- Adds AIC for linear and logistic (resistivity log-log).
- Adds a diagnostic plot (gnuplot) with 3 panels:
    (1) logRho vs logS with data + linear + (optional) logistic
    (2) residuals of linear
    (3) residuals of logistic (if accepted; else empty)
- Keeps v12 logistic-only plot (data + logistic if accepted; otherwise points only)
"""
...
# Logistic constraints (avoid flat fits)
K_MIN = 0.05
K_MAX = 50.0

# Logistic acceptance settings
LOGISTIC_R2_EPS = 0.005   # require at least this R2 improvement unless AIC better
...
def aic_gaussian(ss_res, n, k_params):
    # AIC = n*ln(SSE/n) + 2k  (up to constant)
...
def write_gnuplot_logistic_diagnostics(results_dir: Path, csv_name: str, out_png: str,
    ...
    # residual columns: logRho - model
    # We'll compute using gnuplot expressions from the CSV
    # col5=logS, col6=logRho
    log_defs = ""
...
plot '{csv_name}' using 5:6 with points pt 7 ps 1.6 title 'data', \\
     f1(x) with lines dt 2 lw 3 title 'linear'""" + ("" if not has_log else """,
     fL(x) with lines dt 1 lw 4 title 'logistic (accepted)'""") + """

# --- Panel 2: residuals (linear)
set title 'Residuals: linear (data - linear)'
set grid
set key top right
set xlabel 'log10(Saturation)'
set ylabel 'residual'
plot '{csv_name}' using 5:($6 - f1($5)) with points pt 7 ps 1.4 title 'residuals'

# --- Panel 3: residuals (logistic)
set title 'Residuals: logistic (data - logistic)'
set grid
set key top right
set xlabel 'log10(Saturation)'
set ylabel 'residual'
""" + (f"plot '{csv_name}' using 5:($6 - fL($5)) with points pt 7 ps 1.4 title 'residuals'\n" if has_log else "plot 0/0 notitle\n") + """
...
def accept_logistic(lin_r2, lin_aic, log_fit):
    """
    Decide whether logistic should be plotted/kept.
    """
    if log_fit is None:
        return False, "no_fit"
    ...
    # Must be finite, and not trivially flat (k already bounded, but still)
...
def process_run(run_root: Path, target_freq: float, sample_filter: str|None,
    ...
    # Plots (logistic plotted ONLY if accepted)
    ...
    if not dry_run:
        ...
        subprocess.run(["gnuplot", "plot_logistic_only.gp"], cwd=str(results_dir), check=True,
                       timeout=(gnuplot_timeout if gnuplot_timeout > 0 else None))
        subprocess.run(["gnuplot", "plot_logistic_diagnostics.gp"], cwd=str(results_dir), check=True,
                       timeout=(gnuplot_timeout if gnuplot_timeout > 0 else None))
