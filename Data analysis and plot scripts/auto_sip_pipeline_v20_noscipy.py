#!/usr/bin/env python3
"""
auto_sip_pipeline_v20_noscipy.py

v20:
- Keep RAW linear + optional logistic + separate logistic plots.
- Keep INDEX "goal" plot (blue/red) using through-origin fits on:
    logRI  = log10(rho/rho_ref)   = logRho - logRho_ref
    logICI = log10(sigma_ref/sigma)= logImag_ref - logImag    (FIXED)

- Robust gnuplot:
  * Works even if header lookup fails: uses numeric columns ($6,$7,...).
  * Avoids "no valid points" by never relying on a header-only expression in residual panels.
"""
...
# ---------------- Plot writers (numeric cols; gnuplot-proof) ----------------
...
def write_plot_logistic_diag(results_dir: Path, csv_name: str, out_png: str, target_freq: float, lin_rho, log_fit_acc):
    ...
    panel1 = "plot '{csv}' using 6:7 with points pt 7 ps 1.6 title 'data', \\\n     f1(x) with lines dt 2 lw 3 title 'linear'".format(csv=csv_name)
    if has_log:
        panel1 += ", \\\n     fL(x) with lines dt 1 lw 4 title 'logistic (accepted)'"
    panel1 += "\n"

    panel2 = "plot '{csv}' using 6:($7 - f1($6)) with points pt 7 ps 1.4 title 'residuals'\n".format(csv=csv_name)

    if has_log:
        panel3 = "plot '{csv}' using 6:($7 - fL($6)) with points pt 7 ps 1.4 title 'residuals'\n".format(csv=csv_name)
        extra3 = ""
    else:
        extra3 = (
            "unset key\n"
